...
console.log("Question-One");
 function evennumbersum(n: number): number {
  ...
  function numberstobeprint(numbers: number[]) {
...
function Calculation( radius:number ) :number {
    let pi = 3.14;
...
function Failkarnewala(grades:number[]): number[] {
...
function flg(numbers:number []) : number {
    let large = numbers[0];
    for (let number of numbers) {
      if (number > large){
        large = number;
